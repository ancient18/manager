<template>
    <div id="input-manager">
        ...
        <el-table :data="rawTableData" id="gbox" :useIndex="true" :stripe="true" :border="true" :fit="true"
            ...
            <el-table-column v-for="(col, index) in columns" :key="index" :label="col.label" :prop="col.prop"
                ...
                <template v-slot="{ column, row }">
                    ...
                    <div v-else-if="column.property === 'status'">
                        <el-tag :type="row[column.property] === 7 ? 'success' : ''">{{ row[column.property] === 7 ?
                '生成合同' : '未生成合同'
                            }}</el-tag>
    ...
    methods: {
        ...
        async getContractList() {
            try {
                ...
                const res = await contractList(this.pageOptions);
                if (res.data.code === 20000) {
